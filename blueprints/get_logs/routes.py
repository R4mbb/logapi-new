...
# 허용된 로그 레벨
VALID_LEVELS = {'TRACE', 'INFO', 'DEBUG', 'WARN', 'ERROR', 'FATAL'}

@get_logs_bp.route('/apache2', methods=['GET'])
def get_apache_logs():
    # 필터 조건
    ...
    query = f"SELECT {selected_fields} FROM apache2_logs"
    params = []
...
@get_logs_bp.route('/nginx', methods=['GET'])
def get_nginx_logs():
    # 필터 조건
    level = request.args.get('level')
    limit = request.args.get('limit', 100, type=int)  # 기본 제한 100
    ...
    # SQL 쿼리 생성
    selected_fields = ', '.join(requested_fields)
    query = f"SELECT {selected_fields} FROM nginx_logs"
    params = []
